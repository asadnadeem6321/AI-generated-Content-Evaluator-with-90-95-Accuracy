...
#
#         # Permission check
#         if request.user.is_student() and submission.user != request.user:
#             return Response({'error': 'Access denied'}, status=403)
#         if request.user.is_teacher():
#             if submission.assignment and submission.assignment.class_obj.teacher != request.user:
#                 return Response({'error': 'Access denied'}, status=403)
#
#         # Revoke Celery task without importing celery app directly
#         if submission.task_id:
#             celery.current_app.control.revoke(
#                 submission.task_id, terminate=True, signal='SIGKILL'
#             )
#
#         # Delete existing results
#         if hasattr(submission, 'result'):
#             submission.result.paragraphs.all().delete()
#             submission.result.delete()
#
#         # Reset to fresh state
#         submission.status = 'queued'
#         submission.task_id = None
#         submission.total_paragraphs = 0
#         submission.processed_paragraphs = 0
#         submission.processed_at = None
#         submission.is_paused = False
#         submission.paused_at = None
#         submission.paused_by = None
#         submission.save()
#
#         return Response({'message': 'Processing terminated. Ready for new evaluation.'})
#
#     @action(detail=True, methods=['post'], permission_classes = [IsTeacher])
...
class SubmissionViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if user.is_teacher():
            queryset = Submission.objects.filter(
                Q(assignment__class_obj__teacher=user) |  # 👈 class submissions
                Q(user=user, assignment__isnull=True)  # 👈 teacher's own evaluatio
            )
        elif user.is_student():
            queryset = Submission.objects.filter(user=user)
        else:
    ...
    def perform_create(self, serializer):
        ...
        # queue_submission_processing.delay(
        #     submission_id=str(submission.id),
        #     user_role=self.request.user.role,
        #     is_teacher_view=False
        # )
        task_result = queue_submission_processing.delay(  # 👈 renamed to task_result
            submission_id=str(submission.id),
            user_role=self.request.user.role,
            is_teacher_view=False
        )
        submission.task_id = str(task_result.id)  # 👈 task_result.id
    ...
    def status(self, request, pk=None):
        ...
        return Response({
            ...
            'processing_percentage': submission.processing_percentage,
            'is_complete': submission.status == 'completed',
            'is_paused': submission.is_paused,
            'paused_at': submission.paused_at,
        })
    ...
    def pause(self, request, pk=None):
        ...
        if submission.is_paused:
            return Response({'message': 'Already paused'})
        submission.pause(request.user)
        return Response({'message': 'Submission paused', 'paused_at': submission.paused_at})
    ...
    def resume(self, request, pk=None):
        ...
        return Response({'message': 'Submission resumed', 'pending_paragraphs': pending_count})
    ...
    def terminate(self, request, pk=None):
        ...
        # Set terminated flag — task checks this and stops itself
        submission.status = 'terminated'
        submission.task_id = None
        submission.save(update_fields=['status', 'task_id'])

        # Try revoke anyway (works on Linux, no-op on Windows)
        if submission.task_id:
            celery.current_app.control.revoke(
                submission.task_id, terminate=True, signal='SIGTERM'
            )
    ...
    @action(detail=False, methods=['post'], permission_classes=[IsTeacher])
    def bulk_resume(self, request):
        """Teacher can resume multiple submissions"""
        submission_ids = request.data.get('submission_ids', [])
